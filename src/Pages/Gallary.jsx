...
const Gallary = () => {
    ...
    const fetchGallery = async () => {
        setLoading(true);
        try {
            const result = await AxiosPublic.get(`/gallery?page=${page}&limit=${limit}`);
    ...
    // Reference for last element observer
    ...
    return (
        <div>
            ...
            <div className='flex items-center justify-center flex-col gap-4 '>
                ...
                <button className={`btn ${user ? 'btn-primary' : 'btn-disabled'}`} onClick={() => document.getElementById('my_modal_3').showModal()}>
                    Share
                </button>
            ...
            {loading && <div className="min-h-[100vh] min-w-full flex justify-center items-center"> <span className="loading loading-spinner text-neutral"></span></div>}
        </div>
    );
};
